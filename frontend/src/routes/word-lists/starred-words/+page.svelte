...
<div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm p-4 mb-4">
	<p class="text-sm font-medium text-gray-600 dark:text-slate-400 mb-2">Star a word</p>
...
<div class="flex-1 min-w-0 bg-white dark:bg-slate-900 rounded-lg shadow-sm overflow-hidden">
	...
	{:else if rows.length === 0}
		<p class="text-gray-500 dark:text-slate-400 p-4">No starred words yet - star one above, or from any analysis.</p>
		...
		<table class="w-full">
			<thead class="bg-gray-50 dark:bg-slate-950 border-b border-gray-200 dark:border-slate-800">
				<tr>
					...
					<th class="w-px whitespace-nowrap text-center px-4 py-3 text-sm font-medium text-gray-700 dark:text-slate-300">Starred</th>
				</tr>
			</thead>
			<tbody class="divide-y divide-gray-100 dark:divide-slate-800">
				{#each filtered() as row (row.id)}
					<tr class="cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-800" onclick={(e) => handleRowClick(e, row.word)}>
						<td class="px-4 py-3">
						...
						<td class="w-px whitespace-nowrap px-4 py-3 text-center">
							<button
								...
								class="p-1 rounded text-amber-500 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 disabled:opacity-50"
